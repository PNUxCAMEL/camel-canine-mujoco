...
{
    QMainWindow::setWindowIcon(QIcon(QString(GUI_RSC_DIR) + "openpath_logo.png"));
    QString logoPath = QString(GUI_RSC_DIR) + "logo.png";
...
void MainWindow::DisplayUpdate()
{
    ...
    if (mNewGUIinputSignal)
    {
        ...
        case FSM_INITIAL:
            ...
            ui->LE_TROT_SLOW->setStyleSheet("background-color:pink");
            ui->LE_TROT_STOP->setStyleSheet("background-color:pink");
            ui->LE_ARM_CONTROL_GRP_OPEN->setStyleSheet("background-color:pink");
            ui->LE_ARM_CONTROL_GRP_CLOSE->setStyleSheet("background-color:lightgreen");
            ui->LE_ARM_CONTROL_TELE_ON->setStyleSheet("background-color:pink");
            ui->LE_ARM_CONTROL_TELE_OFF->setStyleSheet("background-color:lightgreen");
            break;
...
//            ui->TB_MOTOR->item(index, 1)->setText(QString().sprintf("%d", sharedMemory->motorErrorStatus[index]));
        ...
        for (int index = 0; index < MOTOR_NUM_ARM; index++)
        {
            ...
            if (sharedMemory->motorStatus)
            {
                ui->TB_MOTOR->verticalHeaderItem(index+MOTOR_NUM_LEG)->setBackground(QColor(75, 75, 255));
            }
            else
            {
                ui->TB_MOTOR->verticalHeaderItem(index+MOTOR_NUM_LEG)->setBackground(QColor(255, 75, 75));
            }

            /// visualize motor error state
            ui->TB_MOTOR->setItem(index+MOTOR_NUM_LEG, 0, new QTableWidgetItem(QString::number(sharedMemory->armMotorErrorStatus[index])));
            ui->TB_MOTOR->item(index+MOTOR_NUM_LEG, 0)->setTextAlignment(Qt::AlignCenter);
//            ui->TB_MOTOR->item(index, 1)->setText(QString().sprintf("%d", sharedMemory->motorErrorStatus[index]));
            if (sharedMemory->armMotorErrorStatus[index] != 0)
            {
                ui->TB_MOTOR->item(index+MOTOR_NUM_LEG, 0)->setBackgroundColor(QColor(255, 100, 100));
            }
            else
            {
                ui->TB_MOTOR->item(index+MOTOR_NUM_LEG, 0)->setBackgroundColor(QColor(100, 255, 100));
            }

            ui->TB_MOTOR->setItem(index+MOTOR_NUM_LEG, 1, new QTableWidgetItem(QString::number(sharedMemory->armMotorTemp[index])));
            ui->TB_MOTOR->item(index+MOTOR_NUM_LEG, 1)->setTextAlignment(Qt::AlignCenter);

            ui->TB_MOTOR->setItem(index+MOTOR_NUM_LEG, 2, new QTableWidgetItem(QString::number(sharedMemory->armMotorVoltage[index], 'f', 1)));
            ui->TB_MOTOR->item(index+MOTOR_NUM_LEG, 2)->setTextAlignment(Qt::AlignCenter);

            ui->TB_MOTOR->setItem(index+MOTOR_NUM_LEG, 3, new QTableWidgetItem(QString::number(sharedMemory->armMotorPosition[index] * R2D, 'f', 1)));
            ui->TB_MOTOR->item(index+MOTOR_NUM_LEG, 3)->setTextAlignment(Qt::AlignCenter);

            ui->TB_MOTOR->setItem(index+MOTOR_NUM_LEG, 4, new QTableWidgetItem(QString::number(sharedMemory->armMotorDesiredTorque[index], 'f', 1)));
            ui->TB_MOTOR->item(index+MOTOR_NUM_LEG, 4)->setTextAlignment(Qt::AlignCenter);

            ui->TB_MOTOR->setItem(index+MOTOR_NUM_LEG, 5, new QTableWidgetItem(QString::number(sharedMemory->armMotorTorque[index], 'f', 1)));
            ui->TB_MOTOR->item(index+MOTOR_NUM_LEG, 5)->setTextAlignment(Qt::AlignCenter);
        }
    }
    else if (ui->TAB_ARM_CONTROL->isVisible())
    {
        ui->LE_ARM_CONTROL_CURRENT_X->setText(QString().sprintf("%.2f m", sharedMemory->currentEndEffectorPosition[0]));
        ui->LE_ARM_CONTROL_CURRENT_Y->setText(QString().sprintf("%.2f m", sharedMemory->currentEndEffectorPosition[1]));
        ui->LE_ARM_CONTROL_CURRENT_Z->setText(QString().sprintf("%.2f m", sharedMemory->currentEndEffectorPosition[2]));
        ui->LE_ARM_CONTROL_CURRENT_RR->setText(QString().sprintf("%.1f deg", sharedMemory->currentEndEffectorEulerAngle[0] * R2D));
        ui->LE_ARM_CONTROL_CURRENT_RP->setText(QString().sprintf("%.1f deg", sharedMemory->currentEndEffectorEulerAngle[1] * R2D));
        ui->LE_ARM_CONTROL_CURRENT_RY->setText(QString().sprintf("%.1f deg", sharedMemory->currentEndEffectorEulerAngle[2] * R2D));

        ui->LE_ARM_CONTROL_GOAL_X->setText(QString().sprintf("%.2f m", sharedMemory->desiredEndEffectorPosition[0]));
        ui->LE_ARM_CONTROL_GOAL_Y->setText(QString().sprintf("%.2f m", sharedMemory->desiredEndEffectorPosition[1]));
        ui->LE_ARM_CONTROL_GOAL_Z->setText(QString().sprintf("%.2f m", sharedMemory->desiredEndEffectorPosition[2]));
        ui->LE_ARM_CONTROL_GOAL_RR->setText(QString().sprintf("%.1f deg", sharedMemory->desiredEndEffectorEulerAngle[0] * R2D));
        ui->LE_ARM_CONTROL_GOAL_RP->setText(QString().sprintf("%.1f deg", sharedMemory->desiredEndEffectorEulerAngle[1] * R2D));
        ui->LE_ARM_CONTROL_GOAL_RY->setText(QString().sprintf("%.1f deg", sharedMemory->desiredEndEffectorEulerAngle[2] * R2D));
        if(sharedMemory->isArmTele)
        {
            ui->LE_ARM_CONTROL_TELE_ON->setStyleSheet("background-color:lightgreen");
            ui->LE_ARM_CONTROL_TELE_OFF->setStyleSheet("background-color:pink");
        }
        else
        {
            ui->LE_ARM_CONTROL_TELE_ON->setStyleSheet("background-color:pink");
...
void MainWindow::on_BT_ARM_CONTROL_X_UP_clicked()
{
    double upperLimit = 0.6;
    double temp;
    temp = sharedMemory->desiredEndEffectorPosition[0] + 0.05;
    sharedMemory->desiredEndEffectorPosition[0] = fmin(temp, upperLimit);
}

void MainWindow::on_BT_ARM_CONTROL_X_DOWN_clicked()
{
    double lowerLimit = 0.4;
    double temp;
    temp = sharedMemory->desiredEndEffectorPosition[0] - 0.05;
    sharedMemory->desiredEndEffectorPosition[0] = fmax(temp, lowerLimit);
}

void MainWindow::on_BT_ARM_CONTROL_Y_UP_clicked()
{
    double upperLimit = 0.2;
    double temp;
    temp = sharedMemory->desiredEndEffectorPosition[1] + 0.05;
    sharedMemory->desiredEndEffectorPosition[1] = fmin(temp, upperLimit);
}

void MainWindow::on_BT_ARM_CONTROL_Y_DOWN_clicked()
{
    double lowerLimit = -0.2;
    double temp;
    temp = sharedMemory->desiredEndEffectorPosition[1] - 0.05;
    sharedMemory->desiredEndEffectorPosition[1] = fmax(temp, lowerLimit);
}

void MainWindow::on_BT_ARM_CONTROL_Z_UP_clicked()
{
    double upperLimit = 0.5;
    double temp;
    temp = sharedMemory->desiredEndEffectorPosition[2] + 0.05;
    sharedMemory->desiredEndEffectorPosition[2] = fmin(temp, upperLimit);
}

void MainWindow::on_BT_ARM_CONTROL_Z_DOWN_clicked()
{
    double lowerLimit = 0.1;
    double temp;
    temp = sharedMemory->desiredEndEffectorPosition[2] - 0.05;
    sharedMemory->desiredEndEffectorPosition[2] = fmax(temp, lowerLimit);
}

void MainWindow::on_BT_ARM_CONTROL_RR_UP_clicked()
{
    double upperLimit = 30 * D2R;
    double temp;
    temp = sharedMemory->desiredEndEffectorEulerAngle[0] + 10 * D2R;
    sharedMemory->desiredEndEffectorEulerAngle[0] = fmin(temp, upperLimit);
}

void MainWindow::on_BT_ARM_CONTROL_RR_DOWN_clicked()
{
    double lowerLimit = - 30 * D2R;
    double temp;
    temp = sharedMemory->desiredEndEffectorEulerAngle[0] - 10 * D2R;
    sharedMemory->desiredEndEffectorEulerAngle[0] = fmax(temp, lowerLimit);
}

void MainWindow::on_BT_ARM_CONTROL_RP_UP_clicked()
{
    double upperLimit = 45 * D2R;
    double temp;
    temp = sharedMemory->desiredEndEffectorEulerAngle[1] + 15 * D2R;
    sharedMemory->desiredEndEffectorEulerAngle[1] = fmin(temp, upperLimit);
}

void MainWindow::on_BT_ARM_CONTROL_RP_DOWN_clicked()
{
     double lowerLimit = - 45 * D2R;
     double temp;
     temp = sharedMemory->desiredEndEffectorEulerAngle[1] - 15 * D2R;
     sharedMemory->desiredEndEffectorEulerAngle[1] = fmax(temp, lowerLimit);
}

void MainWindow::on_BT_ARM_CONTROL_RY_UP_clicked()
{
    double upperLimit = 45 * D2R;
    double temp;
    temp = sharedMemory->desiredEndEffectorEulerAngle[2] + 15 * D2R;
    sharedMemory->desiredEndEffectorEulerAngle[2] = fmin(temp, upperLimit);
}

void MainWindow::on_BT_ARM_CONTROL_RY_DOWN_clicked()
{
    double lowerLimit = - 45 * D2R;
    double temp;
    temp = sharedMemory->desiredEndEffectorEulerAngle[2] - 15 * D2R;
    sharedMemory->desiredEndEffectorEulerAngle[2] = fmax(temp, lowerLimit);
}

void MainWindow::on_BT_ARM_CONTROL_HOME_clicked()
{
    sharedMemory->gamepad.gui.GUIButton = GUI_ARM_HOME;
}

void MainWindow::on_BT_ARM_CONTROL_MOVE_clicked()
{
    sharedMemory->gamepad.gui.GUIButton = GUI_ARM_MOVE;
}

void MainWindow::on_BT_ARM_CONTROL_TELE_ON_clicked()
{
    sharedMemory->gamepad.gui.GUIButton = GUI_ARM_TELE_ON;
}

void MainWindow::on_BT_ARM_CONTROL_TELE_OFF_clicked()
{
    sharedMemory->gamepad.gui.GUIButton = GUI_ARM_TELE_OFF;
}


void MainWindow::on_BT_ARM_CONTROL_GRP_OPEN_clicked()
{
    sharedMemory->gamepad.gui.GUIButton = GUI_ARM_GRP_OPEN;
    ui->LE_ARM_CONTROL_GRP_OPEN->setStyleSheet("background-color:lightgreen");
    ui->LE_ARM_CONTROL_GRP_CLOSE->setStyleSheet("background-color:pink");
}

void MainWindow::on_BT_ARM_CONTROL_GRP_CLOSE_clicked()
{
    sharedMemory->gamepad.gui.GUIButton = GUI_ARM_GRP_CLOSE;
    ui->LE_ARM_CONTROL_GRP_OPEN->setStyleSheet("background-color:pink");
    ui->LE_ARM_CONTROL_GRP_CLOSE->setStyleSheet("background-color:lightgreen");
}